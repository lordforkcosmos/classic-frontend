...
At a high level, PlanqDelta functions just like a normal exchange. Unlike a traditional exchange, which has all of its business logic defined and executed on a private server owned by a company, PlanqDelta's business logic is defined and executed in a smart contract on the public, decentralized [Planq](https://ethereum.org) blockchain. The PlanqDelta GUI (Graphical User Interface) is designed to let you interact with the current smart contract without having to deal with the low-level details of blockchain transactions.

The current smart contract allows you to deposit or withdraw Ether or any [ERC-20](https://github.com/ethereum/EIPs/issues/20) Planq token.

Like any other exchange, PlanqDelta has an order book of resting orders. A resting order consists of a price, volume, expiration time (measured in blocks), and signature. In effect, it represents a signed intent to trade. When you create a new resting order, it gets broadcast to an off-chain order book server. The primary benefit of storing resting orders off-chain is that you don't have to create an Planq transaction and pay gas to submit a resting order. PlanqDelta does have a backup mechanism that allows orders to be submitted with on-chain transactions.
...
The Token interface defines the ERC-20 token standard. PlanqDelta relies on the Token fuction signatures to be able to do token transfers. PlanqDelta's [test framework](https://github.com/etherdelta/etherdelta.github.io/blob/master/test.js) uses the StandardToken implementation along with the ReserveToken contract to implement and trade a basic token.
...
The AccountLevels interface defines a contract that can keep track of account levels for PlanqDelta users. The regular level involves paying make and take fees. The market maker silver level involves paying a take fee, but no make fee, and getting a make rebate. The gold level involves paying a take fee, but no make fee, and getting a make rebate equal to the take fee paid by the counterparty. The test framework uses the AccountLevelsTest contract to test the different account levels.
...
The EtherDelta constructor simply initializes the admin account, fee account, account levels address, and fee percentages. The default function simply throws an error. Any Ether sent to PlanqDelta without a function call will be returned to sender.
